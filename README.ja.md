...
- 行内/右クリックから ID コピー、ローカルタイトルの設定・削除
...
セッションタイトルは別の opt-in です。ビューのtoolbarまたは設定から `agShowSessionId.readTitles` を有効にすると、VS Code のローカルread-only state databaseから `chat.ChatSessionStore.index` 1件だけを読み取ります。タイトルはチャット内容から派生する場合がありますが、メモリ内だけに保持し、ログには記録しません。チャットメッセージやJSONL本文は読み取りません。
...
グローバルコマンドはコマンドパレットの **最近の Copilot Chat ID** カテゴリに表示されます。セッション固有のコピー、詳細、ローカルタイトル操作はActivity Barの各セッション行に表示されます。
...
- デバッグログを読み取らない
- セッションファイルへ書き込まない
- プロンプト、応答、参照、コードを収集しない
- 診断ログには短縮した ID 接頭辞だけを記録し、完全なセッション ID は記録しない
- 任意のタイトルメタデータ取得では限定index 1件だけを読み、セッション ID・タイトル・最終保存時刻だけをメモリ内に保持
